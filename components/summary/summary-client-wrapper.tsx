      ...
      <div className="flex flex-col items-center gap-8">
        ...
        <div className="flex items-center gap-6 text-sm text-muted-foreground mt-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-foreground">Preparing summary</span>
          </div>
        </div>
      </div>
    </div>
...
export function SummaryClientWrapper({ children }: SummaryClientWrapperProps) {
  const [showAnalyzing, setShowAnalyzing] = useState(false)

  useEffect(() => {
    // Check if we're coming from the consultation page with analyzing state
    const isAnalyzing = sessionStorage.getItem("clinisense_analyzing")
    ...
    if (isAnalyzing === "true") {
      ...
      // Show overlay for a brief moment to ensure smooth transition
      const timer = setTimeout(() => {
        setShowAnalyzing(false)
      }, 800) // Short delay to let the page render underneath
